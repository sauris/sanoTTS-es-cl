#!/usr/bin/env bash
# Reduced-size (~340k param) variant for any of the new voices: reuses the
# TEACHER packs from the main run (student-size-independent) and retrains
# only the students smaller. See PIPELINE.md §5.
#   scripts/run_phase3_small_voice.sh <voice> <lang>
set -euo pipefail
VOICE=${1:?voice}
...
TEACHER="$REPO/artifacts/voices/$LANG/$VOICE-medium/$LANG-$VOICE-medium.onnx"
CUT="$RUN/decoder-cut/$LANG-$VOICE-medium-decoder-from-generator-input.onnx"
...
"$PY" "$REPO/tools/export_roota_self_contained_package.py" \
  --package-name "$VOICE-small" --language "$LANG" --voice "$LANG-$VOICE-small" \
  ...
  --piper-config "$REPO/artifacts/voices/$LANG/$VOICE-medium/$LANG-$VOICE-medium.onnx.json" \
  --sample-rate 22050 --duration-length-scale 1.0 \
  --out-dir "$SMALL/package"

echo "PHASE3_SMALL_DONE for $VOICE (total params in $SMALL/package/manifest.json)"
